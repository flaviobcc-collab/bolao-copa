...
function MeusPalpites({user, profile, show}){
  ...
  return <section>
    <div className="toolbar sticky-actions"><span>{fechadoGlobal?'🔒 Palpites bloqueados':`🟢 Palpites abertos até ${fmtDate(config?.limite_palpite)} BRT`}</span><button onClick={saveAll} disabled={saving || !pagamentoConfirmado}>{saving?'Salvando...':'Salvar Palpites'}</button></div>
    {!pagamentoConfirmado && (
      <div className="score-help pagamento-bloqueio">
        🔒 Seus palpites estão bloqueados até a confirmação do pagamento.
      </div>
    )}
...
function AdminParticipantes({show}){
  ...
  async function togglePagamento(r){
    const confirmado = r.pagamento_status === 'confirmado';

    const patch = confirmado
      ? { pagamento_status: 'pendente', pagamento_confirmado_em: null }
      : { pagamento_status: 'confirmado', pagamento_confirmado_em: new Date().toISOString() };
    ...
    show(error
      ? error.message
      : confirmado
        ? 'Confirmação de pagamento removida.'
  ...
  return <div className="panel user-admin-panel">
    <div className="panel-head"><div><h3>Usuários do bolão</h3><p>Bloqueie, desbloqueie, exclua ou confirme manualmente o pagamento dos participantes.</p></div><button onClick={load}>Atualizar</button></div>
    <div className="table-wrap"><table className="users-table"><thead><tr><th>Usuário</th><th>Telefone</th><th>Status</th><th>Pagamento</th><th>Perfil</th><th>Cadastro</th><th>Ações</th></tr></thead><tbody>{rows.map(r=>{ const st=statusLabel(r); const pago = r.pagamento_status === 'confirmado'; return <tr key={r.id} className={r.ativo===false || r.deleted_at ? 'muted-row' : ''}><td><div className="user-cell"><span className="mini-avatar">{initials(r.nome || r.email)}</span><div><strong>{r.nome || 'Sem nome'}</strong><small>{r.email}</small></div></div></td><td>{r.telefone || '—'}</td><td><span className={`user-status ${st.cls}`}>{st.label}</span></td><td><span className={pago ? 'pagamento-ok' : 'pagamento-pendente'}>{pago ? '✅ Confirmado' : '⏳ Pendente'}</span></td><td><span className="role-chip">{r.perfil || 'participante'}</span></td><td>{r.created_at ? fmtDate(r.created_at) : '—'}</td><td className="actions user-actions"><button onClick={()=>role(r.id,r.perfil==='admin'?'participante':'admin')}>{r.perfil==='admin'?'Tornar participante':'Tornar admin'}</button><button onClick={()=>togglePagamento(r)} className={pago ? 'secondary' : ''}>{pago ? '❌ Remover pagamento' : '✅ Confirmar pagamento'}</button><button className={r.ativo===false?'':'secondary'} onClick={()=>toggleAtivo(r)}>{r.ativo===false?<><Unlock size={15}/> Desbloquear</>:<><Ban size={15}/> Bloquear</>}</button><button className="danger" disabled={busy===r.id || r.deleted_at} onClick={()=>excluirUsuario(r)}>{busy===r.id?'Excluindo...':<><Trash2 size={15}/> Excluir</>}</button></td></tr>})}</tbody></table></div>
  </div>
}
