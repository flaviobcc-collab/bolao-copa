...
import { CalendarDays, Eye, Gauge, LogOut, Settings, Trophy, UserRound, Users, Vote, Shield, Flag, ListChecks, ChevronRight, BarChart3, Clock } from 'lucide-react';
import './style.css';
...
function Dashboard({show, user, profile, goRanking}){
  ...
  async function load(){
    ...
    const { data: us } = await supabase.from('participantes').select('*').order('nome'); setUsers(us || []);
  ...
  const saveOne = async (j) => {
    ...
    setSaving(true);
    const { error } = await supabase.from('palpites').upsert([p], { onConflict:'participante_id,jogo_id' });
    setSaving(false);
