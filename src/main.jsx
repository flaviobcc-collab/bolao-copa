...
function Auth({ show }) {
  ...
  const obterLimiteCadastro = () => {
    const valor = config?.limite_cadastro;
    if (!valor) return null;

    const limite = new Date(valor);
    if (Number.isNaN(limite.getTime())) return null;

    // Se a data vier zerada, tratamos como válida até o fim do dia.
    // Isso evita bloquear no começo do dia quando o administrador escolhe só a data.
    if (
      limite.getHours() === 0 &&
      limite.getMinutes() === 0 &&
      limite.getSeconds() === 0
  ...
  useEffect(() => {
    ...
    async function carregarConfig() {
      ...
      if (error) {
        console.error('Erro ao carregar configuração do bolão:', error);
        setConfig({});
        setConfigLoaded(true);
        mensagem('erro', 'Não foi possível carregar a configuração do bolão. O cadastro será liberado temporariamente.');
        return;
      }
  ...
  // 🔒 BLOQUEIO DE CADASTRO - controlado pelo campo limite_cadastro no Admin
  useEffect(() => {
    if (!configLoaded || !abrirCadastro) return;
    ...
    setIsLogin(false);
  }, [configLoaded, config, abrirCadastro]);

  const abrirTelaCadastro = () => {
    setStatusMsg('');

    if (!configLoaded) {
      mensagem('erro', 'Carregando configuração do bolão. Tente novamente em alguns segundos.');
      return;
    }
  ...
  const submit = async (e) => {
    ...
    if (!isLogin) {
      if (!configLoaded) {
        setBusy(false);
        mensagem('erro', 'Aguarde o carregamento da configuração do bolão.');
        return;
      }
    ...
    <div className="auth">
      <form onSubmit={submit} className="auth-card">
        ...
        <button
          type="button"
          className="link"
          onClick={isLogin ? abrirTelaCadastro : voltarParaLogin}
        >
...
function MinhaParticipacao({user, profile, setProfile, show}) {
  ...
  async function atualizarStatus() {
    ...
    if (!error && data) {
      setProfile(data);
      if (data.pagamento_status === 'confirmado') {
  ...
  async function gerarPix() {
    ...
    const data = await gerarPixPagamento({
      user_id: user.id,
    ...
    if (data.error || data.message) {
      show(`Erro ao gerar Pix: ${data.error || data.message}`);
      return;
    }
    ...
    if (!error && atualizado) setProfile(atualizado);

    show('Pix gerado com sucesso. Após o pagamento, aguarde a confirmação automática.');
  }
    ...
    <section className="participacao-panel">
      ...
      <div className="panel-head">
        <div>
          ...
          <p>
            Gere seu Pix para confirmar a participação no bolão. Após o pagamento,
      ...
      <div className="participacao-grid">
        <div className={`participacao-card ${pagamentoConfirmado ? 'confirmado' : 'pendente'}`}>
          <span>Status</span>
      ...
      {pagamentoPendente && (
        <div className="score-help participacao-alerta">
          Seus palpites serão liberados após a confirmação do pagamento.
        </div>
      )}

      <div className="participacao-actions">
        <button onClick={gerarPix} disabled={loading || pagamentoConfirmado}>
          {loading ? 'Gerando Pix...' : pagamentoConfirmado ? 'Pagamento confirmado' : 'Gerar Pix'}
        </button>

        <button className="secondary" onClick={atualizarStatus}>
          Atualizar status
        </button>
      </div>
...
function Dashboard({show, user, profile, goRanking}){
  ...
  async function load(){
    ...
    const { data: meus, error: eMeus } = await supabase
      .from('palpites')
      .select('*')
  ...
  const gruposPorData = useMemo(() => filteredJogos.reduce((acc,j)=>{
  ...
  const setVal = (jogoId, k, v) => setPalpites(p => ({
    ...p,
    [jogoId]: {
      ...(p[jogoId]||{}),
      jogo_id:jogoId,
  ...
  const saveOne = async (j) => {
    ...
    if(!p || p.palpite_a === null || p.palpite_a === undefined || p.palpite_b === null || p.palpite_b === undefined) {
      return show('Informe os dois placares.');
    }

    const row = {
      participante_id: p.participante_id || user.id,
      jogo_id: p.jogo_id || j.id,
      palpite_a: p.palpite_a,
      palpite_b: p.palpite_b
    };
    ...
    <div className="main-column">
      ...
      {datas.map(data=><section className="date-block" key={data}>
        ...
        {gruposPorData[data].map(j=>{
          ...
          return <div className={`match-card-wide status-${info.cls}`} key={j.id}>
            <div className={`status-pill ${info.cls}`}>{info.label}</div>
...
function AdminParticipantes({show}){
 ...
 async function alterarTodosOverride(valor){
  ...
  async function alterarOverride(id, valor) {
    ...
    show(error ? error.message : 'Permissão de palpites atualizada.');
    load();
  }
...
function calcularPremiacao(totalParticipantes, valor, taxa) {
  const total = Number(totalParticipantes || 0) * Number(valor || 0);
  const taxaValor = total * (Number(taxa || 0) / 100);
