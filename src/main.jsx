...
function App() {
  ...
  return <div className="app">
    <aside className="sidebar">
      <div className="brand"><Trophy/> <span>Bolão da Copa</span></div>
...
function MeusPalpites({user, show}){
  ...
  return <section><div className="toolbar"><span>{fechado?'🔒 Palpites encerrados':'🟢 Palpites abertos'}</span><button onClick={saveAll} disabled={fechado}>Salvar Palpites</button></div>{jogos.map(j=>{ const p = palpites[j.id] || {}; const pts = calcPoints(j,p); return <GameCard key={j.id} jogo={j}><div className="scorebox"><input disabled={fechado} type="number" min="0" value={p.palpite_a ?? ''} onChange={e=>setVal(j.id,'palpite_a',e.target.value)} /><span>x</span><input disabled={fechado} type="number" min="0" value={p.palpite_b ?? ''} onChange={e=>setVal(j.id,'palpite_b',e.target.value)} /></div><div className="result-line">Oficial: {j.gols_a ?? '—'} x {j.gols_b ?? '—'} · Pontos: {pts ?? 'aguardando'}</div></GameCard>})}</section>
}
...
function VerPalpites({show}){
  ...
  return <section><div className="toolbar"><label>Participante</label><select value={selected} onChange={e=>setSelected(e.target.value)}>{users.map(u=><option key={u.id} value={u.id}>{u.nome || u.email}</option>)}</select></div>{jogos.map(j=>{ const p=palpites[j.id]; return <GameCard key={j.id} jogo={j}><div className="scorebox read"><span>{p?.palpite_a ?? '—'}</span><span>x</span><span>{p?.palpite_b ?? '—'}</span></div></GameCard>})}</section>
}

function Calendario({show}){
  const [jogos,setJogos]=useState([]); useEffect(()=>{fetchJogos(show).then(setJogos)},[]);
  return <section>{jogos.length===0 && <Empty msg="Nenhum jogo cadastrado."/>}{jogos.map(j=><GameCard key={j.id} jogo={j}><div className="scorebox read"><span>{j.gols_a ?? '—'}</span><span>x</span><span>{j.gols_b ?? '—'}</span></div></GameCard>)}</section>
}
...
function Ranking({show}){
  ...
  return <div className="panel"><table><thead><tr><th>#</th><th>Participante</th><th>Pontos</th><th>Placares exatos</th></tr></thead><tbody>{rows.map((r,i)=><tr key={r.id}><td>{i+1}</td><td>{r.nome || r.email}</td><td>{r.pontos}</td><td>{r.exatos}</td></tr>)}</tbody></table></div>
}
...
function AdminJogos({show}){ const [jogos,setJogos]=useState([]), [grupos,setGrupos]=useState([]), [sel,setSel]=useState([]), [form,setForm]=useState({grupo_id:'',selecao_a_id:'',selecao_b_id:'',data_hora:''}); const load=async()=>{ setJogos(await fetchJogos(show)); const {data:g}=await supabase.from('grupos').select('*').order('ordem'); setGrupos(g||[]); const {data:s}=await supabase.from('selecoes').select('*').order('nome'); setSel(s||[]); }; useEffect(()=>{load()},[]); async function add(){ const row={...form,data_hora:form.data_hora?new Date(form.data_hora).toISOString():null}; const {error}=await supabase.from('jogos').insert(row); show(error?error.message:'Jogo criado.'); load(); } async function genAll(){ for(const g of grupos){ const {data:gs}=await supabase.from('grupo_selecoes').select('selecao_id').eq('grupo_id',g.id); const ids=(gs||[]).map(x=>x.selecao_id); const rows=[]; for(let i=0;i<ids.length;i++) for(let j=i+1;j<ids.length;j++) rows.push({grupo_id:g.id,selecao_a_id:ids[i],selecao_b_id:ids[j],data_hora:null}); if(rows.length) await supabase.from('jogos').insert(rows); } show('Jogos gerados.'); load(); } async function del(id){ if(!confirm('Excluir jogo?'))return; const {error}=await supabase.from('jogos').delete().eq('id',id); show(error?error.message:'Jogo excluído.'); load(); } return <div><div className="toolbar"><button onClick={genAll}>Gerar todos os jogos por grupo</button></div><div className="panel form-grid"><select value={form.grupo_id} onChange={e=>setForm({...form,grupo_id:e.target.value})}><option value="">Grupo</option>{grupos.map(g=><option value={g.id} key={g.id}>{g.nome}</option>)}</select><select value={form.selecao_a_id} onChange={e=>setForm({...form,selecao_a_id:e.target.value})}><option value="">Seleção A</option>{sel.map(s=><option value={s.id} key={s.id}>{s.nome}</option>)}</select><select value={form.selecao_b_id} onChange={e=>setForm({...form,selecao_b_id:e.target.value})}><option value="">Seleção B</option>{sel.map(s=><option value={s.id} key={s.id}>{s.nome}</option>)}</select><input type="datetime-local" value={form.data_hora} onChange={e=>setForm({...form,data_hora:e.target.value})}/><button onClick={add}>Criar jogo</button></div><CalendarioTable jogos={jogos} onDel={del}/></div> }
function CalendarioTable({jogos,onDel}){ return <div className="panel"><table><tbody>{jogos.map(j=><tr key={j.id}><td>Grupo {j.grupo_nome}</td><td>{j.time_a} x {j.time_b}</td><td>{fmtDate(j.data_hora)}</td><td>{onDel&&<button className="danger" onClick={()=>onDel(j.id)}>Excluir</button>}</td></tr>)}</tbody></table></div> }
