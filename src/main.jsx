...
function MeusPalpites({user, show}){
  ...
  const fechadoGlobal = config?.limite_palpite ? new Date() > new Date(config.limite_palpite) : false;
  const setVal = (jogoId, k, v) => setPalpites(p => ({...p, [jogoId]: {...(p[jogoId]||{}), jogo_id:jogoId, participante_id:user.id, [k]: v === '' ? null : Number(v)}}));
  const saveAll = async () => {
    const abertos = jogos.filter(j => !jogoBloqueadoParaPalpite(j, fechadoGlobal)).map(j => j.id);
    const rows = Object.values(palpites).filter(p => abertos.includes(p.jogo_id) && p.palpite_a !== null && p.palpite_a !== undefined && p.palpite_b !== null && p.palpite_b !== undefined);
    if (!rows.length) return show(fechadoGlobal ? 'Prazo geral de palpites encerrado.' : 'Nenhum palpite aberto para salvar.');
    setSaving(true);
...
function Dashboard({show}){
  const [stats,setStats]=useState({jogos:0,participantes:0,palpites:0,aoVivo:0,finalizados:0});
  useEffect(()=>{(async()=>{ const jogos = await fetchJogos(show); const [{count:u},{count:p}] = await Promise.all([supabase.from('participantes').select('*',{count:'exact',head:true}),supabase.from('palpites').select('*',{count:'exact',head:true})]); setStats({jogos:jogos.length,participantes:u||0,palpites:p||0,aoVivo:jogos.filter(j=>j.status==='EM_ANDAMENTO').length,finalizados:jogos.filter(j=>j.status==='FINALIZADO').length}); })()},[]);
  return <><div className="hero"><div><span>Bolão Esportivo</span><h2>Seu bolão da Copa 2026 em modo competição.</h2><p>Palpites, ranking e controle de jogos em tempo real.</p></div><Trophy size={54}/></div><div className="cards"><KPI label="Jogos" value={stats.jogos}/><KPI label="Participantes" value={stats.participantes}/><KPI label="Palpites salvos" value={stats.palpites}/><KPI label="Ao vivo" value={stats.aoVivo}/><KPI label="Finalizados" value={stats.finalizados}/></div></>
}
