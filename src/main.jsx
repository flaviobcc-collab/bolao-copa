...
function GameCard({jogo, children}){
  return <div className={`game-card ${jogo.gols_a !== null && jogo.gols_a !== undefined && jogo.gols_b !== null && jogo.gols_b !== undefined ? 'finished' : ''}`}>
    <div className="game-meta"><span>{jogo.partida_numero ? `Jogo ${jogo.partida_numero} · ` : ''}Grupo {jogo.grupo_nome || '-'}</span><span>{fmtDate(jogo.data_hora)} BRT</span></div>
...
function Calendario({show}){
  ...
  return <section>{jogos.length===0 && <Empty msg="Nenhum jogo cadastrado."/>}{datas.map(data=><div className="date-group" key={data}><h2>{fmtDay(gruposPorData[data][0]?.data_hora)}</h2>{gruposPorData[data].map(j=><GameCard key={j.id} jogo={j}><div className="scorebox read official"><span>{j.gols_a ?? '—'}</span><span>x</span><span>{j.gols_b ?? '—'}</span></div><div className="result-line">{j.gols_a === null || j.gols_a === undefined || j.gols_b === null || j.gols_b === undefined ? 'Jogo em aberto' : 'Resultado oficial informado'}</div></GameCard>)}</div>)}</section>
}
...
function AdminJogos({show}){
  ...
  return <div>
    ...
    <div className="panel form-grid">
      ...
      <input type="datetime-local" value={form.data_hora} onChange={e=>setForm({...form,data_hora:e.target.value})}/><input placeholder="Estádio" value={form.estadio||''} onChange={e=>setForm({...form,estadio:e.target.value})}/><input placeholder="Cidade" value={form.cidade||''} onChange={e=>setForm({...form,cidade:e.target.value})}/>
    ...
    {edit && <div className="panel edit-panel">
      ...
      <div className="form-grid">
        ...
        <input type="number" min="0" placeholder="Gols B" value={edit.gols_b} onChange={e=>setEdit({...edit,gols_b:e.target.value})}/><input placeholder="Estádio" value={edit.estadio||''} onChange={e=>setEdit({...edit,estadio:e.target.value})}/><input placeholder="Cidade" value={edit.cidade||''} onChange={e=>setEdit({...edit,cidade:e.target.value})}/>
      </div>
...
function CalendarioTable({jogos,onEdit,onDel}){ return <div className="panel table-wrap"><table><thead><tr><th>Nº</th><th>Grupo</th><th>Jogo</th><th>Data/Hora BRT</th><th>Local</th><th>Placar</th><th>Ações</th></tr></thead><tbody>{jogos.map(j=><tr key={j.id}><td>{j.partida_numero || '—'}</td><td>Grupo {j.grupo_nome}</td><td>{j.time_a} x {j.time_b}</td><td>{fmtDate(j.data_hora)}</td><td>{[j.estadio,j.cidade].filter(Boolean).join(' · ') || '—'}</td><td>{j.gols_a ?? '—'} x {j.gols_b ?? '—'}</td><td className="actions">{onEdit&&<button onClick={()=>onEdit(j)}>Editar</button>}{onDel&&<button className="danger" onClick={()=>onDel(j.id)}>Excluir</button>}</td></tr>)}</tbody></table></div> }
